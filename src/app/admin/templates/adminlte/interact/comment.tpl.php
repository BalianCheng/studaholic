...
<div class="nav-tabs-custom">
    <ul class="nav nav-tabs pull-right">
        ...
        foreach ($contentNav as $navName => $navTxt) {
            ...
            echo $wrap->a($navTxt, $this->url("interact:comment", array('t' => $navName)));
        }
        ?>
        <li class="pull-left header">
            <i class="fa fa-th"></i>文章评论
        </li>
    </ul>
            ...
            <?php if (!empty($data['list'])) : ?>
                <table class="table table-bordered table-hover">
                    <tr>
                        ...
                        <th style="width:60px;min-width:60px;">文章ID</th>
                    ...
                    <?php foreach ($data['list'] as $d) : ?>
                        <tr>
                            ...
                            <td style="min-width:260px;max-width:300px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">
                                <?php echo $this->imagesToLink($d['comment_content'], $d['title_id']) ?>
                            </td>
                            <td><?php echo long2ip($d['comment_ip']) ?></td>
                            <td><?php echo date('Y-m-d H:i:s', $d['comment_time']) ?></td>
                            <td>
                                <?php
                                switch ($this->params['t']) {
                                    case 'hidden':
                                        $cancelHiddenAct = $this->url('interact:changeStatus', array('type' => 'comment', 'id' => $d['comment_id'], 'status' => 1));
                                        echo $this->a('取消折叠', $cancelHiddenAct);

                                        $blockAct = $this->url('interact:changeStatus', array('type' => 'comment', 'id' => $d['comment_id'], 'status' => -2));
                                        echo $this->a('屏蔽', $blockAct);
                                        break;

                                    case 'block':
                                        $cancelBlockAct = $this->url('interact:changeStatus', array('type' => 'comment', 'id' => $d['comment_id'], 'status' => 1));
                                        echo $this->a('取消屏蔽', $cancelBlockAct);

                                        $hiddenAct = $this->url('interact:changeStatus', array('type' => 'comment', 'id' => $d['comment_id'], 'status' => -1));
